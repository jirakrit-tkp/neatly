...
export default function BookingHistoryPage() {
    ...
    (async () => {
      try {
        ...
        const uniquePromoCodes = [
          ...new Set(rows.map((r) => r.promo_code).filter(Boolean)),
        ] as string[];
        ...
        const mapped: Booking[] = rows.map((row) => {
          ...
          const roomCount =
            typeof row.room_count === "number" && row.room_count > 0
              ? row.room_count
              : 1;

          // แสดง (N Rooms) ต่อท้ายชื่อห้อง
          const roomCountText = `(${roomCount} Room${
            roomCount > 1 ? "s" : ""
          })`;
          ...
          if (Array.isArray(row.special_requests)) {
            specialReqItems = buildItemsFromArray(
              row.special_requests as unknown[]
            );
          } else if (
            ...
            try {
              ...
              if (Array.isArray(parsed))
                specialReqItems = buildItemsFromArray(parsed as unknown[]);
              else if (parsed)
                specialReqItems = buildItemsFromArray([parsed] as unknown[]);
            } catch {
          ...
          const nightly =
            typeof roomsObj?.promotion_price === "number"
              ? roomsObj.promotion_price
              : 0;
          items.unshift({
            label: `${roomName} ${roomCountText}`,
            amount: nightly * nights * roomCount,
          });
          ...
          const hasEarlyCheckIn = labelsLower.some((l) =>
            /early\s*check[- ]?in/i.test(l)
          );
          const hasLateCheckOut = labelsLower.some((l) =>
            /late\s*check[- ]?out/i.test(l)
          );
          ...
          const cancelledAtText = cancelled
            ? fmtDateUTC(row.cancellation_date)
            : undefined;
  ...
  return (
    <Layout>
      ...
      <main
        ref={mainRef}
        className="max-w-[1120px] mx-auto md:mt-20 md:px-6 pb-5 pt-[80px] sm:pt-[96px] md:pt-12"
      >
